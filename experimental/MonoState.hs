...
|]
type SComp1 h f s a =
  ((h' `MonoHandles` Get) s, (h' `MonoHandles` Put) s, f h s a ~ h') => Comp h' a
--forwardState' :: s -> SComp1 h ForwardState s a -> Comp h a  -- (MonoHandles h Get s, MonoHandles h Put s, h ~ ForwardState h' s a) => s -> Comp h a -> Comp h' a
forwardState' :: s -> SComp' (ForwardState h s a) s a -> Comp h a
forwardState' s c = forwardState s c
[operation|exists s.LogPut :: s -> ()|]
[handler|
  forward h.(MonoHandles h Get s, MonoHandles h Put s, MonoHandles h LogPut s) =>
    PutLogger s a :: a
      monohandles {Put s} where
        Return  x     -> return x
...
[handler|
  forward h.
    LogPutReturner s a :: (a, [s])
      monohandles {LogPut s} where
        Return x   -> return (x, [])
        LogPut s k -> do {(x, ss) <- k (); return (x, s:ss)}
|]
[handler|
  forward h.(h `Handles` PrintLine, Show s) =>
    LogPutPrinter s a :: a
      monohandles {LogPut s} where
        Return x   -> return x
...
--putLogger :: SComp s a -> ((h `MonoHandles` LogPut) s) => SComp' h s a
--putLogger :: (((h `MonoHandles` LogPut) s) => SComp' h s a) -> ((h `MonoHandles` LogPut) s) => SComp' h s a
...
type SEComp s a =
  ((h `MonoHandles` Get) s, (h `MonoHandles` Put) s, h `PolyHandles` Err) =>
     Comp h a
